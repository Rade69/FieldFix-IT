...
def _issue_steps(issue: Issue) -> list[tuple[str, str, str]]:
    color = _SEVERITY_COLOR.get(issue.severity, "#9aa4b2")
    ...
    return [
        ("Evidence", evidence_icon, "#3fb950" if issue.evidence else "#9aa4b2"),
        ("Cause", cause_icon, "#3fb950" if issue.likely_cause else "#9aa4b2"),
        ("Action", action_icon, "#3fb950" if issue.recommended_actions else "#9aa4b2"),
...
class DecisionAssistantWidget(QFrame):
    ...
    def __init__(self) -> None:
        ...
        self._problem_label.setStyleSheet("color: #9aa4b2; font-size: 11px;")
        ...
        self._conclusion_label.setStyleSheet("color: #9aa4b2; font-weight: 600;")
    ...
    def update_data(self, issues: tuple[Issue, ...]) -> None:
        if not issues:
            ...
            self._conclusion_label.setStyleSheet("color: #3fb950; font-weight: 600;")
            self._open_details_button.setEnabled(False)
